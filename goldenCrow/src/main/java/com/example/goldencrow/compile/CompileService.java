...
//        System.out.println("명령어 : "+ Arrays.toString(cmd));
    ...
    */
    public String createDockerfile(String filePath, Long teamSeq, int type) {
        ...
        if (!rightPath) {
            return "Error: Wrong file path(not /home/ubuntu/crow_data/)";
        }
        // teamSeq가 있는지 확인
        Optional<TeamEntity> existTeam = teamRepository.findByTeamSeq(teamSeq);
        if (!existTeam.isPresent()) {
            return "Error: Can't Search team";
        }
        // teamSeq랑 filePath에 있는 숫자랑 같은지 확인
        String[] pathList = filePath.split("/");
        if (Long.parseLong(pathList[4]) != teamSeq) {
            return "Error: Wrong file Path(teamSeq)";
        }
...
//                    "RUN pip3 install -r requirements.txt\n" +
    ...
    public String pyCompile(Map<String, Object> req, Long teamSeq) {
        ...
        if (imageBuild.isEmpty()) {
            return "Error: Can't build docker image";
        }
...
//        System.out.println(Arrays.toString(command));
        ...
        if (container.isEmpty()) {
            return "Error: Can't run docker container";
        }
        String portString = portNum(container);
        if (portString.isEmpty()) {
            return "Error: 런 시켰는데 컨테이너가 안돌아가서 포트를 찾을 수가 없음";
        }
    ...
    public String pyCompileStop(Map<String, String> req) {
        ...
        if (stopedCon.isEmpty() || !stopedCon.equals(conAndImgName)) {
            return "Error: Can't stop conatiner " + conAndImgName;
        }
...
//        System.out.println("삭제한 이미지명 : " + rmImg);
        if (rmImg.isEmpty() || rmImg.startsWith("Error")) {
            return "Error: Can't remove image " + conAndImgName;
        }
        ...
        if (!deleted) {
            return "Error: Can't remove Dockerfile";
        }
        return "SUCCESS";
    }
}
